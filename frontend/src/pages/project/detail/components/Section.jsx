...
export default function Section({ title, subtitle, color, children, collapsible = false, forceOpen = false, filled = true, layerTodos = [], headerRight = null }) {
  ...
  return (
    <div style={{ marginBottom: isOpen ? 28 : 4 }}>
                ...
                >
                  ...
                  {layerTodos.map((todo, i) => {
                    ...
                    return (
                      <div
                        key={todo.id}
                        onClick={() => { setShowTasks(false); setEditingTodo(todo); }}
                        style={{
                          padding: "10px 14px",
                        ...
                        onMouseEnter={(e) => { e.currentTarget.style.background = theme.surfaceAlt || theme.border + "30"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}
                      >
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 5 }}>
                          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: theme.text, fontFamily: F, textDecoration: isDone ? "line-through" : "none" }}>
                            {todo.title}
                          </p>
                          <span style={{ fontSize: 10, color: theme.textFaint, fontFamily: F, flexShrink: 0, marginLeft: 8, marginTop: 2 }}>Edit →</span>
                        </div>
      ...
      {/* Edit modal — opens when a task row is clicked in the popover */}
      {editingTodo && (
        <TodoModal
          initial={editingTodo}
